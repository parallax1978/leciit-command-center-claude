...
for (const width of [360, 390, 768, 1440]) {
  ...
  m.smallTargets.length === 0 ? pass(`${width}: all targets >= 44px`) : fail(`${width}: small targets ${JSON.stringify(m.smallTargets)}`)
  {
    ...
    // The oklab-interpolated gradient is lowered at build time to equivalent multi-stop sRGB stops.
    const isBrandGradient = (img) => /^linear-gradient\(to right(?: in oklab)?, rgb\(106, 19, 207\)(?:, rgb\([^)]+\)){0,4}, rgb\(199, 30, 146\)\)$/.test(img)
    const colorOk = isBrandGradient(hb.bgImage) && hb.color === 'rgb(255, 255, 255)' && hb.shadow !== 'none'
    sizeOk && colorOk && hb.centered && !hb.overflow ? pass(`${width}: hero button ${hb.w}x${hb.h}, ${hb.font}/${hb.weight}, brand gradient with shadow, centered`) : fail(`${width}: hero button ${JSON.stringify(hb)}`)
    ...
    offerBtn.bg === 'rgb(255, 255, 255)' && offerBtn.bgImage === 'none' && offerBtn.color === 'rgb(106, 19, 207)' ? pass(`${width}: pricing button is white with purple text`) : fail(`${width}: pricing button ${JSON.stringify(offerBtn)}`)
    others.filter((b) => b.p !== 'offer').every((b) => isBrandGradient(b.bgImage) && b.color === 'rgb(255, 255, 255)') ? pass(`${width}: header and closing buttons use the brand gradient`) : fail(`${width}: gradient buttons ${JSON.stringify(others)}`)
